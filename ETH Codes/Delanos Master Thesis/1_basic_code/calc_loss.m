function [M_Magn,IVL_tot,IVL_tot_PROB_norm,tot_loss,prob,losses,exrate,mCasSum_final,num_DG_final]=calc_loss(M_Mmin,M_dm,M_Mmax,sGMPE,sAMPL,sDISTR,sCOST,model,Mmin,Mmax)
% function [M_Magn,IVL_tot,IVL_tot_PROB_norm,tot_loss,prob,losses,exrate,mCasSum_final,num_DG_final]=calc_loss(M_Mmin,M_dm,M_Mmax,sGMPE,sAMPL,sDISTR,sCOST,model,Mmin,Mmax)


VCprob = 3;

%% Borehole/Earthquake Coordinates
xquake = 611643;    % CH x-coordinates of earthquake    
yquake = 270420;    % CH x-coordinates of earthquake    
depth_quake = 4700; % borehole/earthquake depth in [m]
depth_quake_km = depth_quake/1000; % borehole/earthquake depth in [km]
...
mVi=importdata('vulnerability_classes_serianex_vi_all.csv');   % all Vi (Vmin, V-, Vo, V+, Vmax)
mVi_EMS=importdata('vulnerability_classes_serianex_vi_EMS.csv');
% attributed EMS vulnerability class (A,B,C,D,E) to building classes, 
% important for casualty calculation
mVi_EMS=char(mVi_EMS); % conversioni of mVi_EMS to characters
...
% casualty matrices according to HAZUS
load casualty_rates.mat

% definition of all magnitudes Mw to calculate
...
% filename_det = ['det_int3.5_6.5_' sGMPE '_' sDISTR '_' sAMPL '_' sCOST '_Mmax_' M_Mmax_name '.mat'];
% save(filename_det);
% 
% ECOS-09 calibrated
% load det_int3.5_6.5_ECOS-09_BinPDF_AMPyes_Expert_SER_Mmax_7.mat
% 
% % Allen-2012 calibrated
% load det_int4_6.5_Allen-2012_BinPDF_AMPyes_SED_Del_Mmax_7.mat

%% some matrix adjustment for the final output...
%% concerning damage grades and casualties
% rearrangment of final casualty matrix
% 1: iMag
% 2: iC
% 3: iVC
% 4: iLoc
for iMag=1:size(mCasSum_allMagVC,1)
    for iLoc=1:size(mCasSum_allMagVC,2)
        for iVC=1:size(mCasSum_allMagVC,3)
            for iC=1:size(mCasSum_allMagVC,4)
                mCasSum_final(iMag,iC,iVC,iLoc)=mCasSum_allMagVC(iMag,iLoc,iVC,iC);
            end
        end
    end
end

% rearrangement of damage grades
for iVC=1:size(mVi,2)
    for iMag=1:length(M_Magn)
    DG0(iMag,1) = sum(num_build_per_DG(1,iVC,:,iMag));
    DG1(iMag,1) = sum(num_build_per_DG(2,iVC,:,iMag));
    DG2(iMag,1) = sum(num_build_per_DG(3,iVC,:,iMag));
    DG3(iMag,1) = sum(num_build_per_DG(4,iVC,:,iMag));
    DG4(iMag,1) = sum(num_build_per_DG(5,iVC,:,iMag));
    DG5(iMag,1) = sum(num_build_per_DG(6,iVC,:,iMag));
    end
    num_DG_final(:,:,iVC) = [DG0 DG1 DG2 DG3 DG4 DG5];
    % 1: iDG
    % 2: iMag
    % 3: iVC
end
...
prob=IVL_tot(2:end,VCprob).*C_forecast_magbin';
...
for Loc=1:size(building_stock,2);
...
%     Int4(Loc)=GMPE_fIobs_allLocs_allMags(Loc,ind4(Loc));
    ...
    IVL1(Loc)=IVL_tot_allLocs_allMags(Loc,VCprob,ind1(Loc));
    IVL2(Loc)=IVL_tot_allLocs_allMags(Loc,VCprob,ind2(Loc));
    IVL3(Loc)=IVL_tot_allLocs_allMags(Loc,VCprob,ind3(Loc));
    IVL4(Loc)=IVL_tot_allLocs_allMags(Loc,VCprob,ind4(Loc));
...
% figure;
% loglog(losses./10^6,exrate,'ko-','LineWidth',2); grid on;
% set(gca,'LineWidth',2,'FontSize',24,'FontWeight','normal','FontName','Times')
...
for ii=1:length(M_Magn);
    for Loc=1:length(GMPE_dist);
            matris(Loc,ii)=IVL_tot_allLocs_allMags(Loc,VCprob,ii);
    end
end
