...
class BasicOperation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.callback = MagicMock(name='MockCallback')
        self.time_step = 1/TEST_SPEED
        self.simulator = Simulator(self.history, self.callback)
    ...
    def test_delivery(self):
        ...
        while self.t_elapsed < timeout and self.num_callbacks < 3:
            self.app.processEvents()
            self.step_time()
            self.num_callbacks = self.callback.call_count
        self.assertLess(self.t_elapsed, 4.5/TEST_SPEED,
    ...
    def test_pause(self):
        ...
        while self.t_elapsed < 10/TEST_SPEED and self.num_callbacks < 3:
            self.app.processEvents()
            self.num_callbacks = self.callback.call_count
    ...
    def test_stop(self):
        ...
        while self.t_elapsed < 5/TEST_SPEED and self.num_callbacks < 3:
            self.app.processEvents()
            self.num_callbacks = self.callback.call_count
            if self.t_elapsed == 2/TEST_SPEED:
                self.simulator.stop()
            self.step_time()
        self.assertEqual(self.num_callbacks, 2)
