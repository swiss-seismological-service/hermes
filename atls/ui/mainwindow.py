...
class MainWindow(QtGui.QMainWindow):
    ...
    def action_show_timeline(self):
        if self.timeline_window is None:
            self.timeline_window = TimelineWindow(atls_core=self.atls_core,
                                                  parent=self)
        self.timeline_window.show()

    def action_show_sim_controls(self):
        if self.simulation_window is None:
            self.simulation_window = SimulationWindow(atls_core=self.atls_core,
                                                      parent=self)
    ...
    def update_status(self):
        ...
        if self.atls_core.simulator.state == SimulatorState.RUNNING:
            state_msg = 'Simulating'
        elif self.atls_core.simulator.state == SimulatorState.PAUSED:
