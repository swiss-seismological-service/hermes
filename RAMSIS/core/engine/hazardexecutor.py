...
def set_statuses(forecast_id, status):
    ...
    for scenario in forecast.scenarios:
        ...
        logger.info(f"#####################3scenario state: {scenario.status.state}")
    logger.info(f"forecast state: {forecast.status.state}")
    session.commit()
    session.close()

class PrepareHazardForForecast(Task):
    def run(self, forecast_id: int, data_dir: str) -> List[ScenarioHazardPreparation]:
...
class MapScenarioRuns(Task):
    ...
    def run(self, hazard_preparation: ScenarioHazardPreparation) -> List[HazardRunInfo]:
        # Scenario_level
...
class ExecuteHazardRun(Task):
    def run(self, hazard_preparation: HazardRunInfo):
        ...
        # TODO set the hazard model run status and leave the forecast to another task. handle errors!
