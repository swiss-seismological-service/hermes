...
class ImportHydAppError(AppError):
    """AppError ({})."""
...
class ImportHydApp:
    """
    Import hydraulics (i.e. well/hydraulics).
    """
    VERSION = '0.1'
    ...
    def build_parser(self, parents=[]):
        ...
        """
        parser = CustomParser(
            prog="ramsis-import-hyd",
            description='Import hydraulics.',
            parents=parents)
        ...
        parser.add_argument('--force', '-f', action='store_true',
                            default=False,
                            help='Overwrite existing hydraulics.')
    ...
    def run(self):
        ...
        with session_scope() as session:
            try:
                ...
                raise ImportHydAppError(str(err))
            ...
            for section in bh.sections:
                self.logger.debug('Number of hydraulic samples: '
                                  f'{len(section.hydraulics)}')
            self.logger.info(f'Imported hydraulics {bh!r}.')
...
def main():
    ...
    app = ImportHydApp(log_id='RAMSIS')
