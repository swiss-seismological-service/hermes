...
model_config_path = join(
    resources_path, 'model_forge_2022_bayes.json')
...
class TestInducedForgeBayesCase:
    def test_ramsis_forge_bayes_setup(self, mocker, session):
    ...
    @pytest.mark.run(after='test_ramsis_forge_bayes_setup')
    def test_run_forge_bayes_forecast(self, mocker, session):
