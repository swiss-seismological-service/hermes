...
Note that *Controller* is not an actual object in RT-RAMSIS but rather a collection of window controller classes named xxx\ *window.py*. The name *Controller* is used in the diagram above in a conceptual sense. In RT-RAMSIS there is also a :class:controller: **class** which is actually part of the core. 
...
* **core** The :doc:`core` package contains most of the business logic of RT-RAMSIS. It's main module defines the :class:`Controller`: class which owns the main components of RT-RAMSIS and serves as the entry point for the user interface actions. :class:`Controller` controls the forecasting :doc:`engine`, the :doc:`scheduler` the :doc:`simulator`. It also manages the currently active :doc:`project`.
